...
{\bf Exercise 7.1.3.}
Show that a collection of events $\{ A_i\}_{i \in I}$ are independent if and only if for any finite $J \subseteq I$,
\begin{align*}
...
\begin{align*}
\mathbb{P}\left(\bigcup^\infty_{n=1}L_n\cap B\right)&=\mathbb{P}\left(\bigcup^\infty_{n=1}(L_n\cap B)\right) \\
...
&=\mathbb{P}(B)\mathbb{P}\left(\bigcup^\infty_{n=1}L_n\right) && (L_n\text{  disjoint}).
\end{align*}
...
\noindent{\bf Exercise 7.1.10.}
\label{7.1.10}
If $\{X_i\}_{i\in I}$ is a collection of independent random variables, then show that for any disjoint subsets $J, K \subseteq I$, the $\sigma$-algebras generated by
...
Then by Exercise 7.1.9, all the elements in $\mathcal{P}_X$ are independent to all the elements in $\mathcal{P}_Y$ implies $\sigma(\mathcal{P}_X)$ and $\sigma(\mathcal{P}_Y)$ are independent. Hence $\sigma(\{X_i\}_{i\in I})$ and $\sigma(\{Y_j\}_{j\in J})$ are independent.
$\hspace{\fill}\square$\\
% \end{answer}
